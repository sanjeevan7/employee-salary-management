  ...
  try {
    ...
    if (shouldTryFallback) {
      logger.log(
        ...
        "createMany not found on data provider (you may need to implement it see: https://github.com/sanjeevan7/employee-salary-management#reducing-requests): using fallback instead"
      );
      try {
        const items = await createInDataProviderFallback(dataProvider, resource, values);
        reportItems.push(...items);
  ...
  try {
    ...
    if (shouldTryFallback) {
      logger.log(
        ...
        "updateManyArray not found on data provider (you may need to implement it see: https://github.com/sanjeevan7/employee-salary-management#reducing-requests): using fallback instead"
      );
      try {
